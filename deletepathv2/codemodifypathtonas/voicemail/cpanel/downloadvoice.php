...
$uuid = $_REQUEST[id];
                ...
                //$sql.=" FROM v_xml_cdr  a  where ".$_SESSION['ssql'];
					...
					if (!is_null($uuid)) {
                        ...
                        $sql = "select * from v_voicemail_messages as m, v_voicemails as v, v_domains as d ";
                ...
                if (file_exists($path.'/msg_'.$voicemail_message_uuid.'.mp3')) {
...
//exit();
				...
				//download the file
					if (file_exists($record_file)) {
						...
						switch ($file_ext) {
							case "wav" : header('Content-Disposition: attachment; filename="msg_'.$voicemail_message_uuid.'.wav"'); break;
							case "mp3" : header('Content-Disposition: attachment; filename="msg_'.$voicemail_message_uuid.'.mp3"'); break;
							case "ogg" : header('Content-Disposition: attachment; filename="msg_'.$voicemail_message_uuid.'.ogg"'); break;
						}
						}
						else {
