	...
	class voicemail {
						...
						//loop through voicemail ids
							if (is_array($voicemail_ids) && @sizeof($voicemail_ids) != 0) {
								...
								$file_path = $_SESSION['switch']['voicemail']['dir']."/default/".$_SESSION['domain_name']."/".$voicemail_id;
											...
											elseif($checktime > $thirty){
											$file_path = '/mnt/recording/voice_backup/storage/voicemail/default/'.$_SESSION['domain_name'].'/'.$voicemail_id;
											}
											//$file_path = $_SESSION['switch']['voicemail']['dir']."/default/".$_SESSION['domain_name']."/".$voicemail_id;
		...
		public function message_delete() {
			...
			//delete the recording
				$voicemail_time = $this->created_epoch;
			date_default_timezone_set('Asia/Bangkok');
			$time_now = strtotime(date('Y-M-d H:i:s'));
			$thirty = (30*24*60*60);
			$checktime = $time_now - $voicemail_time;
			...
			elseif($checktime > $thirty){
			$file_path = '/mnt/recording/voice_backup/storage/voicemail/default/'.$_SESSION['domain_name'].'/'.$this->voicemail_id;
			}	
		...
		public function message_download() {
			...
			elseif($checktime > $thirty){
			$path = '/mnt/recording/voice_backup/storage/voicemail/default/'.$_SESSION['domain_name'].'/'.$this->voicemail_id;
			}
				// $path = $_SESSION['switch']['voicemail']['dir'].'/default/'.$_SESSION['domain_name'].'/'.$this->voicemail_id;
