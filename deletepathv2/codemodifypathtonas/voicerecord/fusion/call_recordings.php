	...
	if (is_array($call_recordings) && @sizeof($call_recordings) != 0) {
		...
		foreach ($call_recordings as $row) {
			...
			//	if (file_exists($row['call_recording_path'].'/'.$row['call_recording_name'])) {
				$answer = date_create($row['call_recording_date']);
				$Year = date_format($answer,"Y");
				$Mount = date_format($answer,"M");
				$Day = date_format($answer,"d");
				$pathnas = "/mnt/recording/voice_backup/recordings/cns6.cans.cc/archive/".$Year."/".$Mount."/".$Day;
			//echo $pathnas;	
				if ((file_exists($row['call_recording_path'].'/'.$row['call_recording_name'])) or (file_exists($pathnas.'/'.$row['call_recording_uuid'].'.mp3'))) {
					if (permission_exists('call_recording_play')) {
