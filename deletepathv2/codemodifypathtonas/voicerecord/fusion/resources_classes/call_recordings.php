			...
			if (permission_exists('call_recording_play') || permission_exists('call_recording_download')) {
				...
				//get call recording from database
					if (is_uuid($this->recording_uuid)) {
						$sql = "select call_recording_name, call_recording_path, call_recording_base64,call_recording_date,call_recording_uuid ";
						$sql .= "from v_call_recordings ";
						$sql .= "where call_recording_uuid = :call_recording_uuid ";
						...
						if (is_array($row) && @sizeof($row) != 0) {
							...
							$call_recording_uuid = $roe['call_recording_uuid'];
					...
					$check_date = date_create($row['call_recording_date']);
					$date_format = date_format($check_date,"M j, Y ,h:i:s A");
					$thirty = (30*24*60*60) ;
					$timebuildvoice = strtotime($date_format);
					$checktime = $time_now - $timebuildvoice;
				$Year = date_format($check_date,"Y");
				$Mount = date_format($check_date,"M");
				$Day = date_format($check_date,"d");	
				...
				elseif($checktime > $thirty){
					$full_recording_path = "/mnt/recording/voice_backup/recordings/cns6.cans.cc/archive".$Year."/".$Mount."/".$Day."/".$call_recording_uuid.".mp3";
					$default_path = "/mnt/recording/voice_backup/recordings/cns6.cans.cc";
				}
